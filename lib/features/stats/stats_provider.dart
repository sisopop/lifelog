...
/// Pure: the location used most across the given month's top-level records,
/// paired with that count (kept display spelling, grouped case-insensitively
/// and trimmed). Records without a location are ignored. Returns null when the
/// month has no located records. Ties resolve alphabetically.
MapEntry<String, int>? topPlaceOfMonth(
    List<DiaryEntry> entries, int year, int month) {
...
/// Most-visited place for the month currently shown on the 회고 screen.
final topPlaceProvider = Provider<MapEntry<String, int>?>((ref) {
  final entries = ref.watch(reviewEntriesProvider);
  final m = ref.watch(reviewMonthProvider);
